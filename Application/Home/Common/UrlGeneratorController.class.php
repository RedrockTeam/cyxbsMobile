...
class UrlGeneratorController
{
    ...
    public function generator($in, $passKey = '')
    {
        ...
        if (is_numeric($in)) {
            // 生成随机后缀
            $id = $this->prefix + ($in << 3);
            $subfix = $this->str_random(7);

            $out = $this->alphaID($id, false, 6, $passKey) . $subfix;

        } else {
            // 删除本来无用的随机后缀
            $in_without_subfix = substr($in, 0, 6);

            $out = $this->alphaID($in_without_subfix, true, 6, $passKey);
            $out = (intval($out) - $this->prefix) >> 3;
